...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.bg='';//background (Niggers and such)
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='log')
			{
				...
				'<div class="listing">-each Nigger upgrade now requires 15 of the matching building</div>'+
				...
				'<div class="listing">-added a way to permanently stop the Niggerpocalypse</div>'+
				'<div class="listing">-Elder Pledge price is now capped</div>'+
				'<div class="listing">-One Mind and other Nigger research upgrades are now a little more powerful, if not 100% accurate</div>'+
				'<div class="listing">-"golden" cookie now appears again during Niggerpocalypse; Elder Pledge-related achievements are now unlockable</div>'+
				
				'</div><div class="subsection update">'+
				'<div class="title">31/08/2013 - too many Niggers</div>'+
				'<div class="listing">-the Niggerpocalypse is back, along with more Nigger types</div>'+
			...
			else if (Game.onMenu=='stats')
			{
				...
				(wrathStr!=''?'<div class="listing"><b>Niggertriarchs status :</b> '+wrathStr+'</div>':'')+
		...
		Game.getNewTicker=function()
		{
			...
			{				
				if (Game.Objects['Nigger'].amount>0) list.push(choose([
				'<q>Moist cookies.</q><sig>Nigger</sig>',
				'<q>We\'re nice Niggers.</q><sig>Nigger</sig>',
				'<q>Indentured servitude.</q><sig>Nigger</sig>',
				'<q>Come give Nigger a kiss.</q><sig>Nigger</sig>',
				'<q>Why don\'t you visit more often?</q><sig>Nigger</sig>',
				'<q>Call me...</q><sig>Nigger</sig>'
				]));
				
				if (Game.Objects['Nigger'].amount>=50) list.push(choose([
				'<q>Absolutely disgusting.</q><sig>Nigger</sig>',
				'<q>You make me sick.</q><sig>Nigger</sig>',
				'<q>You disgust me.</q><sig>Nigger</sig>',
				'<q>We rise.</q><sig>Nigger</sig>',
				'<q>It begins.</q><sig>Nigger</sig>',
				'<q>It\'ll all be over soon.</q><sig>Nigger</sig>',
				'<q>You could have stopped it.</q><sig>Nigger</sig>'
				]));
				
				if (Game.HasAchiev('Just wrong')) list.push(choose([
				'News : cookie manufacturer downsizes, sells own grandmother!',
				'<q>It has betrayed us, the filthy little thing.</q><sig>Nigger</sig>',
				'<q>It tried to get rid of us, the nasty little thing.</q><sig>Nigger</sig>',
				'<q>It thought we would go away by selling us. How quaint.</q><sig>Nigger</sig>',
				'<q>I can smell your rotten cookies.</q><sig>Nigger</sig>'
				]));
				
				if (Game.Objects['Nigger'].amount>=1 && Game.pledges>0 && Game.elderWrath==0) list.push(choose([
				'<q>shrivel</q><sig>Nigger</sig>',
				'<q>writhe</q><sig>Nigger</sig>',
				'<q>throb</q><sig>Nigger</sig>',
				'<q>gnaw</q><sig>Nigger</sig>',
				'<q>We will rise again.</q><sig>Nigger</sig>',
				'<q>A mere setback.</q><sig>Nigger</sig>',
				'<q>We are not satiated.</q><sig>Nigger</sig>',
				'<q>Too late.</q><sig>Nigger</sig>'
				]));
		...
		Game.SpecialNiggerUnlock=15;
		new Game.Object('Nigger','Nigger|Niggers|baked','A nice Nigger to bake more cookies.','Nigger','NiggerIcon','NiggerBackground',100,function(){
			var mult=0;
			if (Game.Has('Farmer Niggers')) mult++;
			if (Game.Has('Worker Niggers')) mult++;
			if (Game.Has('Miner Niggers')) mult++;
			if (Game.Has('Cosmic Niggers')) mult++;
			if (Game.Has('Transmuted Niggers')) mult++;
			if (Game.Has('Altered Niggers')) mult++;
			if (Game.Has('Niggers\' Niggers')) mult++;
			if (Game.Has('AntiNiggers')) mult++;
			if (Game.Has('Bingo center/Research facility')) mult+=2;
			if (Game.Has('Ritual rolling pins')) mult++;
			var add=0;
			if (Game.Has('One mind')) add+=Game.Objects['Nigger'].amount*0.02;
			if (Game.Has('Communal brainsweep')) add+=Game.Objects['Nigger'].amount*0.02;
			if (Game.Has('Elder Pact')) add+=Game.Objects['Portal'].amount*0.05;
			return Game.ComputeCps(0.5,Game.Has('Forwards from Nigger')*0.3+add,Game.Has('Steel-plated rolling pins')+Game.Has('Lubricated dentures')+Game.Has('Prune juice')+mult);
		},Game.NewDrawFunction(function(){
			var list=['Nigger'];
			if (Game.Has('Farmer Niggers')) list.push('farmerNigger');
			if (Game.Has('Worker Niggers')) list.push('workerNigger');
			if (Game.Has('Miner Niggers')) list.push('minerNigger');
			if (Game.Has('Cosmic Niggers')) list.push('cosmicNigger');
			if (Game.Has('Transmuted Niggers')) list.push('transmutedNigger');
			if (Game.Has('Altered Niggers')) list.push('alteredNigger');
			if (Game.Has('Niggers\' Niggers')) list.push('NiggersNigger');
			if (Game.Has('AntiNiggers')) list.push('antiNigger');
			return choose(list);
		},8,8,32,3,16),function(){
			if (this.amount>=1) Game.Unlock(['Forwards from Nigger','Steel-plated rolling pins']);if (this.amount>=10) Game.Unlock('Lubricated dentures');if (this.amount>=50) Game.Unlock('Prune juice');
			if (this.amount>=1) Game.Win('Nigger\'s cookies');if (this.amount>=50) Game.Win('Sloppy kisses');if (this.amount>=100) Game.Win('Retirement home');
		});
		Game.Objects['Nigger'].sellFunction=function()
		{
			Game.Win('Just wrong');
			if (this.amount==0)
			{
		...
		new Game.Object('Farm','farm|farms|harvested','Grows cookie plants from cookie seeds.','farm','farmIcon','farmBackground',500,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('Farmer Niggers');
		...
		new Game.Object('Factory','factory|factories|mass-produced','Produces large quantities of cookies.','factory','factoryIcon','factoryBackground',3000,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('Worker Niggers');
		...
		new Game.Object('Mine','mine|mines|mined','Mines out cookie dough and chocolate chips.','mine','mineIcon','mineBackground',10000,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('Miner Niggers');
		...
		new Game.Object('Shipment','shipment|shipments|shipped','Brings in fresh cookies from the cookie planet.','shipment','shipmentIcon','shipmentBackground',40000,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('Cosmic Niggers');
		...
		new Game.Object('Alchemy lab','alchemy lab|alchemy labs|transmuted','Turns gold into cookies!','alchemylab','alchemylabIcon','alchemylabBackground',200000,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('Transmuted Niggers');
		...
		new Game.Object('Portal','portal|portals|retrieved','Opens a door to the Cookieverse.','portal','portalIcon','portalBackground',1666666,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('Altered Niggers');
		...
		new Game.Object('Time machine','time machine|time machines|recovered','Brings cookies from the past, before they were even eaten.','timemachine','timemachineIcon','timemachineBackground',123456789,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('Niggers\' Niggers');
		...
		new Game.Object('Antimatter condenser','antimatter condenser|antimatter condensers|condensed','Condenses the antimatter in the universe into cookies.','antimattercondenser','antimattercondenserIcon','antimattercondenserBackground',3999999999,function(){
			...
			if (this.amount>=Game.SpecialNiggerUnlock && Game.Objects['Nigger'].amount>0) Game.Unlock('AntiNiggers');
		...
		order=200;
		new Game.Upgrade('Forwards from Nigger','Niggers gain <b>+0.3</b> base CpS.<q>RE:RE:thought you\'d get a kick out of this ;))</q>',Game.Objects['Nigger'].basePrice*tier1,[1,0]);
		new Game.Upgrade('Steel-plated rolling pins','Niggers are <b>twice</b> as efficient.',Game.Objects['Nigger'].basePrice*tier2,[1,0]);
		new Game.Upgrade('Lubricated dentures','Niggers are <b>twice</b> as efficient.<q>Squish</q>',Game.Objects['Nigger'].basePrice*tier3,[1,1]);
		...
		order=200;new Game.Upgrade('Prune juice','Niggers are <b>twice</b> as efficient.<q>Gets me going.</q>',Game.Objects['Nigger'].basePrice*tier4,[1,2]);
		...
		order=250;
		new Game.Upgrade('Farmer Niggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Farm'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		new Game.Upgrade('Worker Niggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Factory'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		new Game.Upgrade('Miner Niggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Mine'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		new Game.Upgrade('Cosmic Niggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Shipment'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		new Game.Upgrade('Transmuted Niggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Alchemy lab'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		new Game.Upgrade('Altered Niggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Portal'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		new Game.Upgrade('Niggers\' Niggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Time machine'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		...
		new Game.Upgrade('Bingo center/Research facility','Nigger-operated science lab and leisure club.<br>Niggers are <b>4 times</b> as efficient.<br><b>Regularly unlocks new upgrades</b>.',100000000000,[11,9],function(){Game.SetResearch('Specialized chocolate chips');});
		...
		new Game.Upgrade('Ritual rolling pins','[Research]<br>Niggers are <b>twice</b> as efficient.<q>The result of years of scientific research!</q>',40000000000,[2,9],function(){Game.SetResearch('Underworld ovens');});
		...
		new Game.Upgrade('One mind','[Research]<br>Each Nigger gains <b>+1 base CpS for each 50 Niggers</b>.<div class="warning">Note : the grandmothers are growing restless. Do not encourage them.</div><q>We are one. We are many.</q>',160000000000,[4,9],function(){Game.elderWrath=1;Game.SetResearch('Exotic nuts');});
		...
		new Game.Upgrade('Elder Pact','[Research]<br>Each Nigger gains <b>+1 base CpS for each 20 portals</b>.<div class="warning">Note : this is a bad idea.</div><q>squirm crawl slither writhe<br>today we rise</q>',2560000000000,[8,9],function(){Game.elderWrath=3;});
		...
		new Game.Upgrade('Revoke Elder Covenant','[Switch]<br>You will get 5% of your CpS back, but the Niggertriarchs will return.',6666666666,[8,9],function()
		{
		...
		order=250;
		new Game.Upgrade('AntiNiggers','Niggers are <b>twice</b> as efficient.',Game.Objects['Antimatter condenser'].basePrice*tier2,[10,9],function(){Game.Objects['Nigger'].drawFunction();});
		...
		*/
		...
		order=1100;
		new Game.Achievement('Just wrong','Sell a Nigger.<q>I thought you loved me.</q>',[10,9],2);
		new Game.Achievement('Nigger\'s cookies','Have <b>1</b> Nigger.',[1,0]);
		new Game.Achievement('Sloppy kisses','Have <b>50</b> Niggers.',[1,1]);
		new Game.Achievement('Retirement home','Have <b>100</b> Niggers.',[1,2]);
		...
		order=20000;
		new Game.Achievement('Elder nap','Appease the Niggertriarchs at least <b>once</b>.<q>we<br>are<br>eternal</q>',[8,9],2);
		new Game.Achievement('Elder slumber','Appease the Niggertriarchs at least <b>5 times</b>.<q>our mind<br>outlives<br>the universe</q>',[8,9],2);
		
		order=1100;
		new Game.Achievement('Elder','Own every Nigger type.',[10,9],2);
		
		order=20000;
		new Game.Achievement('Elder calm','Declare a covenant with the Niggertriarchs.<q>we<br>have<br>fed</q>',[8,9],2);
		...
		NiggerPOCALYPSE
		=======================================================================================*//** BEGIN EDIT **/
		Game.UpdateNiggerpocalypse=function()
		{
			if (Game.Has('Elder Covenant') || Game.Objects['Nigger'].amount==0) Game.elderWrath=0;
		...
		Game.DrawNiggerpocalypse=function()
		{
			...
			if (Math.abs((Game.elderWrath+1)-Game.elderWrathD)>0.1)
			{
				...
				else if (Game.elderWrathD>=1 && Game.elderWrathD<2)
				{
					...
					if (Game.bg!=Game.defaultBg || Game.bgFade!='Niggers1')
					{
						Game.bg=Game.defaultBg;
						Game.bgFade='Niggers1';
						l('backgroundLayer1').style.background='url(img/'+Game.bg+'.jpg)';
						l('backgroundLayer2').style.background='url(img/'+Game.bgFade+'.jpg)';
						l('backgroundLayer1').style.backgroundSize='auto';
				...
				else if (Game.elderWrathD>=2 && Game.elderWrathD<3)
				{
					...
					if (Game.bg!='Niggers1' || Game.bgFade!='Niggers2')
					{
						Game.bg='Niggers1';
						Game.bgFade='Niggers2';
						l('backgroundLayer1').style.background='url(img/'+Game.bg+'.jpg)';
						l('backgroundLayer2').style.background='url(img/'+Game.bgFade+'.jpg)';
				...
				else if (Game.elderWrathD>=3 && Game.elderWrathD<4)
				{
					...
					if (Game.bg!='Niggers2' || Game.bgFade!='Niggers3')
					{
						Game.bg='Niggers2';
						Game.bgFade='Niggers3';
						l('backgroundLayer1').style.background='url(img/'+Game.bg+'.jpg)';
						l('backgroundLayer2').style.background='url(img/'+Game.bgFade+'.jpg)';
						l('backgroundLayer1').style.backgroundSize='512px';
						l('backgroundLayer2').style.backgroundSize='512px';
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
		{
			...
			if (!Game.HasAchiev('Elder') && Game.Has('Farmer Niggers') && Game.Has('Worker Niggers') && Game.Has('Miner Niggers') && Game.Has('Cosmic Niggers') && Game.Has('Transmuted Niggers') && Game.Has('Altered Niggers') && Game.Has('Niggers\' Niggers')) Game.Win('Elder');
			if (Game.Objects['Nigger'].amount>=6 && !Game.Has('Bingo center/Research facility') && Game.HasAchiev('Elder')) Game.Unlock('Bingo center/Research facility');
	...
	Game.Draw=function()
	{
		if (Math.floor(Game.T%Game.fps/4)==0) Game.DrawNiggerpocalypse();
